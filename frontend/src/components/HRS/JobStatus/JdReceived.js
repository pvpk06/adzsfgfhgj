...
const HrLeads = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await apiService.get(`/api/hr-view-jobs?hrId=${HrId}`); // Adjust the URL as needed
  ...
  const handleResumeDownload = async (jobId) => {
    try {
      ...
      const response = await apiService({
        url: `/api/download-resume/${jobId.jobId}`, // Update the endpoint to match your backend
        method: 'GET',
    ...
    <>
      ...
      <Container fluid className="p-5 " style={{width:'90vw'}}>
        ...
        <Row className="mb-3">
          ...
          {selectedIds.length>0 && <Col md={6} className="d-flex justify-content-end">
